...
class Velero(Embarcacion, IRegateable):
    ...
    NUM_MASTILES_MAX = 4
    NUM_MASTILES_MIN = 1
    VELOCIDAD_MIN = 2  # nudos
    VELOCIDAD_MAX = 30  # nudos
    ...
    def __init__(
        ...
        if nombre is None and num_mastiles is None and num_max_tripulantes is None:
            ...
            nombre = f"Velero {Velero.__num_veleros} "
        ...
        # Constructor CON parámetros
            ...
            if (
                num_mastiles < Velero.NUM_MASTILES_MIN
                or num_mastiles > Velero.NUM_MASTILES_MAX
            ):
                raise ValueError(
    ...
    def iniciar_navegacion(self, velocidad, rumbo, patron, num_tripulantes):
        ...
        if velocidad < self.VELOCIDAD_MIN or velocidad > self.VELOCIDAD_MAX:
            raise Exception(
                f"La velocidad de navegación de {velocidad} nudos es incorrecta."
            )

        # --- VALIDACIÓN EXTRA DEL RUMBO ---
        if rumbo is None:
            raise Exception(
                "El rumbo no puede ser nulo, debes indicar el rumbo (ceñida o empopada)."
            )

        if rumbo not in ("ceñida", "empopada"):
            raise Exception(
                "El rumbo no es correcto, debes indicar el rumbo (ceñida o empopada)."
            )
    ...
    def iniciar_regata(self, otro_velero=None):
        ...
        if not self.is_navegando():
            raise Exception(
                f"No se puede iniciar la regata, el barco {self.get_nombre_barco()} no está navegando."
            )

        if not otro_velero.is_navegando():
            raise Exception(
                f"No se puede iniciar la regata, el barco {otro_velero.get_nombre_barco()} no está navegando."
            )
        ...
        if self.get_num_mastiles() != otro_velero.get_num_mastiles():
            raise Exception(
                f"No se puede iniciar la regata, los barcos "
                f"{self.get_nombre_barco()} y {otro_velero.get_nombre_barco()} "
    ...
    def señalizar(self):
        print(
            f"AVISO del velero {self.get_nombre_barco()} con banderas de señalización marítima."
        )
    ...
    def __str__(self):
        ...
        return f"{base}, Número de mástiles: {self.get_num_mastiles()}"
